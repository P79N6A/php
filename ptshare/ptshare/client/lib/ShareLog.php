...
class ShareLog
{
    ...
    public static function getInstance()
    {
        if (! isset(self::$instance)) {
            $start_time = defined('SHARE_START_TIME') ? SHARE_START_TIME : microtime(true) * 1000;
            if (empty($GLOBALS['SHARE_LOG'])) {
                $log_config = array(
                    'level' => self::LOG_LEVEL_ALL,
                    'split' => self::LOG_SPLIT_DAY,
                    'log_file' => dirname(dirname(__FILE__)) . '/logs/share.sdk.log'
                );
            } else {
                $log_config = $GLOBALS['SHARE_LOG'];
            }
