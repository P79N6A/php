...
class MySQL
{
    ...
    public static function getInstance($config = array())
    {
        ...
        if(empty($config)) {
            throw new MySQLException("配置不能为空", -1001);    
        }
        
        $key = md5(serialize($config));
    
        if(!isset($instance[$key]) || !($instance[$key] instanceof MySQL)) {
            $instance[$key] = new MySQL($config);
        }
	...
	{
		$this->_connect();
        $logSql = $this->getBindedSql($sql, $params);

        Logger::log('sql_query', Util::dbNow(),  [$logSql]);

		if($this->_debug) {
			print $this->getBindedSql($sql, $params)."\n";
		}

		$stmt = new MySQLStatement($this->_conn->prepare($sql));
		if(is_array($params)) {
			if(!empty($params)) {
				$i = 0;
	...
	private function _process($sql, $params)
	{
		...
		if(in_array(preg_replace("/\s{2,}/", " ", strtolower($sql)), array("begin", "commit", "rollback", "start transaction", "set autocommit=0", "set autocommit=1"))) {
			throw new MySQLException("为避免操作异常，请使用包装后的事务处理接口[startTrans, commit, rollback]");
		}

		if($this->_transaction) {
			if($this->_reconnected) {
				throw new MySQLException("数据库链接已丢失!");
			} else {
				try {
					...
					throw new MySQLException($e->errorInfo[2], $e->errorInfo[1]);
				}
			}
		} else {
			try {
	...
	private function _checkSafe($sql, $is_open_safe = true)
	{
		...
		if(!$is_safe) {
			throw new MySQLException("SQL语句:[$sql],存在SQL注入漏洞隐患，请改用bind方式处理或关闭sql执行safe模式.");
		}
	...
	private function _operate($table, $record, $operate, $condition = "", $params = array())
	{
		if(in_array($operate, array("insert", "replace", "update"))) {
			...
			if(empty($fields)) {
				throw new MySQLException("\$record 操作数据必须使用关联数组形式");
			}
		}
	...
	{
		if($this->_transaction) {
			throw new MySQLException("之前开启的事务尚未结束，事务处理不能嵌套操作!");
		}
		...
		try {
			...
			throw new MySQLException($errorInfo[2], $errorInfo[1]);
		}
	...
	{
		if(!$this->_transaction) {
			throw new MySQLException("之前开启的事务已经被提交或没有开启，请仔细查看事务处理过程中的操作语句!");
		}
