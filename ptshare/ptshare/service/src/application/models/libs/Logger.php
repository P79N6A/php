...
class Logger
{
    ...
    public static function getInstance()
    {
        if (self::$instance === null) {
            ...
            self::$instance = new Logger($GLOBALS['LOG'], $intStartTime);
        }
    ...
    public function writeLogs($intLevel, $str, $errno = 0, $arrArgs = null, $depth = 0)
    {
        ...
        $str = sprintf("%s: time[%s] [%s:%d] errno[%d] ip[%s] logId[%u] uri[%s] time_used[%d] %s\n%s", $strLevel, date('m-d H:i:s', time()), $file, $line, $errno, self::getClientIP(), $this->intLogId, isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '', $intTimeUsed, $str, $strArgs);
