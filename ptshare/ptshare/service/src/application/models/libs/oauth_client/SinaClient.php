<?php
include_once dirname(__FILE__).DIRECTORY_SEPARATOR. "sdk". DIRECTORY_SEPARATOR. "saetv2.ex.class.php";
class SinaClient
{
 /* {{{ */
    private $_obj;
 ...
 /* }}} */
    ...
    { /* {{{ */
        return $this->_oauth->getAccessToken("code", array(
            "code" => $code,
 ...
 /* }}} */
    ...
    { /* {{{ */
        $uid_info = $this->_callMethod("get_token_info", array(
            $this->_token
        ));
        ...
        $user = $this->_callMethod("show_user_by_id", array(
            $this->_uid
        ));
 ...
 /* }}} */
    ...
    { /* {{{ */
        $data = $this->_callMethod("friends_ids_by_id", array(
            $this->_uid,
            $cursor,
            $num
        ));
 ...
 /* }}} */
    ...
    { /* {{{ */
        ...
        if (isset($data['error_code']) && isset($data['error'])) {
            if (substr($data['error_code'], 0, 3) == "213" || in_array($data['error_code'], array(
                10006,
                10013
            ))) {
