...
class Follow
{
    ...
    public static function addFollow($uid, $fids)
    { /* {{{ */
        if (! is_array($fids)) {
            $fids = array(
                $fids
        ...
        $followed = self::_addFollow($uid, $fids, "");
    ...
    { /* {{{ */
        try {
            ...
            $dao_following->modFollowing($fid, array(
                "notice" => $notice
            ));
            $dao_follower->modFollower($uid, array(
                "notice" => $notice
            ));
    ...
    { /* {{{ */
        ...
        if ($cache->exists(Follow::KEY_PREFIX . $uid)) {
            ...
            foreach ($followed_list as $k => $v) {
                $followed = Follow::isFollowed($k, array(
                    $uid
                ));
                if ($v && $followed[$uid]) {
                    $friend_list[$k] = true;
