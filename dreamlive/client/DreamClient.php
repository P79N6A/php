<?php
define('DREAM_START_TIME', microtime(true) * 1000);
define('DREAM_CLIENT_PATH', dirname(__FILE__));

date_default_timezone_set('Asia/Shanghai');

require_once DREAM_CLIENT_PATH . '/config/DreamConfig.php';
require_once DREAM_CLIENT_PATH . '/lib/DreamLog.php';
require_once DREAM_CLIENT_PATH . '/lib/DreamRequest.php';

$GLOBALS['DREAM_LOG'] = array(
    'level' => DreamLog::LOG_LEVEL_WARNING, // 日志级别为警告级别，同时业务日志（调试级别）将关闭
    'split' => DreamLog::LOG_SPLIT_DAY,
    'log_file' => DREAM_CLIENT_PATH . '/logs/dream_client.log'
);
...
class DreamClient
{
    ...
    {
        $this->request = DreamRequest::getInstance();
    }
    ...
    {
        $client = DreamClient::getInstance();
    ...
    {
        $api_conf=DreamConfig::conf($name);
        if (! $api_conf) {
        ...
        //$api_conf = DreamConfig::$api_conf[$name];
    ...
    {
        $app_name=isset($_REQUEST['app_name'])?$_REQUEST['app_name']:'zhuiMeng';
        $params = array(
            'userid' => $this->userid,
            'platform' => "server",
