...
{                      
	...
	cmd="svn export -r$revision --force $svn_url $local_dir"
...
function check_files_diff()
{
	...
	do
        ...
        cecho "diff -Bb $svn_src_dir/$file $this_host_src_dir/$file"
...
function upload_src_to()
{
	...
	echo $SCP $src_tgz $host:$uploaded_src_tgz 
	...
	echo $SSH $host "$EXPORT_LANGUAGE;mkdir -p $realdst;tar xvfz $uploaded_src_tgz -C $realdst" 2>&1 | sed -e 's/^/   /'
	$SSH $host "$EXPORT_LANGUAGE;mkdir -p $realdst;tar xvfz $uploaded_src_tgz -C $realdst" 2>&1 | sed -e 's/^/   /'
...
function sudo_ssh_run()
{
	...
	ssh -t $host -p "echo $sudo_password | sudo -S $cmd"
	result=$?
	if [ $result -ne 0 ]
	then
