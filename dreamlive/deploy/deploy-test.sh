...
DEPLOY_TOOLS_DIR=${DEPLOY_TOOLS_DIR/service_branches/}
...
if test ! -e $DEPLOY_TOOLS_DIR/config/conf.test.sh
then
    cecho "$DEPLOY_TOOLS_DIR/config/conf.test.sh 文件不存在" $c_error
    exit 0;
fi
. $DEPLOY_TOOLS_DIR/config/conf.test.sh
