...
if($_CITY_LIST) {
    foreach($_CITY_LIST as $city){
        $bucket_element->cleanElement($_BUCKET_NAME_BASIC.'_'.$city.'_'.$operation_type);
    }
}
echo "clean end \n";
...
while($page<$page_total){
    ...
    ProcessClient::getInstance("dream")->addTask("same_city_slave", $data);
    $page++;
}
...
echo "通知后台 刷新城市列表"," \n";
ProcessClient::getInstance("dream")->addTask("same_city_refresh_list", $cityconfig);
...
if($others) {//其他
    $bucket_name = $_BUCKET_NAME_BASIC.'_其他_'.$operation_type;
    foreach ($others as $k => $v) {
        $paramothers[] = "'".$_BUCKET_NAME_BASIC.'_'.$v.'_'.$operation_type."'";
    }
    $str = implode(',', $paramothers);
    $sql = "update `bucket_element` set bucket_name = '{$bucket_name}' where bucket_name in({$str});";
    // echo $sql, "\n";
    $dao_bucket_element->Execute($sql, null, false);

    // $sql = "SELECT relateid, score, extends FROM `bucket_element` where bucket_name in({$str});";

    // $dao_bucket_element = new DAOBucketElement();
    // $elements = $dao_bucket_element->getAll($sql,null, false);
    ...
    // $dao_bucket_element->import($bucket_name, $importData);
}
