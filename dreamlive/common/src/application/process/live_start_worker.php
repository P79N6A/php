...
class LiveStartBroadCast
{
    ...
    static public function execute($value)
    {
        ...
        Logger::log("live_start_broadcast_room", "get task", array("liveid"=>$liveid, "uid"=>$userid, "nickname"=>$userinfo["nickname"], "line"=>__LINE__));
        ...
        Logger::log("live_start_broadcast_room", "get follower num succ", array("liveid"=>$liveid, "uid"=>$userid, "follow_num"=>count($follow_num), "line"=>__LINE__));
        // 查询 主播粉丝个数 拆分任务 分发任务
        if (!$follow_num) {
            $log_info = array(
            'liveid'    => $liveid,
            ...
            Logger::log("live_start_broadcast_room", "follower empty", array("liveid"=>$liveid, "uid"=>$userid, "follow_num"=>$follow_num, "line"=>__LINE__));
        } else {
            ...
            for ($i = 1; $i <= $task_num; $i++) {
                ...
                Logger::log("live_start_broadcast_room", "OK", array("liveid"=>$liveid, "uid"=>$userid,"offset"=>$offset, "step"=>self::EACH_STEP, "text"=>$text, "nickname"=>$userinfo["nickname"],"vr"=> json_encode($extends) . json_encode($userinfo), "ret"=>$ret, "line"=>__LINE__));
            }
            $log_info = array(
            'liveid'    => $liveid,
            "userid"    => $userid,
