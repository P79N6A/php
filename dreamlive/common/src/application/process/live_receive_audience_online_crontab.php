...
class UsersWatchLiveOnline extends DAOProxy
{
    ...
    {
        $sql = "SELECT * FROM " . $this->getTableName() . "  order by addtime asc limit $offset,$step" ;
...
try {
    $usersWatchLiveOnline = new UsersWatchLiveOnline();
    $all_list = $usersWatchLiveOnline->getAllList($startime_half, 0, 500);
    
    foreach ($all_list as $val) {
        if (!in_array($val['relateid'], $list)) {
            var_dump($val['relateid'] .'--del2--' .$val['uid']);
            $option = array('relateid' => $val['relateid'],'uid' => $val['uid'], "type" => 'force');
            ProcessClient::getInstance("dream")->addTask("live_receive_audience_online_delete", $option);
        }
    }
