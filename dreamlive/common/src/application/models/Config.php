...
class Config
{
    ...
    public function getConfigs($region, $names, $platform, $min_version, $max_version = null)
    {
        $min_version = self::convertVersion($min_version);
        $max_version = self::convertVersion($max_version);

        return $this->_dao_config->getConfigs($region, $names, $platform, $min_version, $max_version);
    }
