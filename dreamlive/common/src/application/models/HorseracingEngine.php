...
class HorseracingEngine
{
    ...
    const SYSTEM_CURRENCY=2;//币种
    const SYSTEM_JOURNAL_TYPE=Account::TRADE_TYPE_GAME_RUN;//帐变类型TRADE_TYPE_GAME_RUN
    const NO_STAKE_BANKER_REFUND_JOURNAL_TYPE=21;//帐变类型
    ...
    const MESSAGE_TAG_START_GAME=2000;//开始游戏消息,推给虚拟聊天室
    const MESSAGE_TAG_RUN_HORSE=2001;//开始跑马消息，推给虚拟聊天室
    const MESSAGE_TAG_GAME_OVER=2002;//游戏结束消息，三押注最多，三贡献最多，推给直播间
    const MESSAGE_TAG_SETTLE_TO_STAKE=2003;//结算推给押注或坐庄者的私信
    const MESSAGE_TAG_SETTLE_MAX_TO_LIVE=2004;//结算推给直播间的赢钱最多用户信息
    const MESSAGE_TAG_BANKER=2005;//抢庄消息
    const MESSAGE_TAG_SWITCH_GAME=2006;//直播间中途推送消息，使端上切换入口
    const MESSAGE_TAG_OPEN_ROOM=2007;//开启虚拟聊天室
    const MESSAGE_TAG_BANKER_REFUND=2008;//抢庄被顶退钱消息
    const MESSAGE_TAG_STAKE=2009;//押注消息
    ...
    {
        $game_dao=new DAOGame();
        $game=$game_dao->getGameByType(DAOGame::TYPE_HORSERACING);
    ...
    public function startGame()
    {
        ...
        $result=$round_dao->addRound(0, 0, 0, DAOHorseracingRound::GAME_STATUS_BANKER, $this->orderid, $options, $mod_time, $start_time, $end_time);
    ...
    public function robotBanker()
    {
        ...
        $game=$this->game;
        $gameid=$game['gameid'];
        GameRobots::robotsBanker($gameid, $amount);
    ...
    public function startStake()
    {
        ...
        $result=$round_dao->updateRound($this->roundid, '', '', '', DAOHorseracingRound::GAME_STATUS_STAKE, '', [], '');
    ...
    public function startRun()
    {
        ...
        $factor=2;
        $one_speed=$winno==DAOHorseracingRound::TRACK_ONE?$base_speed+$factor:$base_speed-$factor;
        $two_speed=$winno==DAOHorseracingRound::TRACK_TWO?$base_speed+$factor:$base_speed-$factor;
        //设置跑马中状态和赛道号
        $round_dao=new DAOHorseracingRound();
        ...
        $result=$round_dao->updateRound($this->roundid, '', '', $winno, DAOHorseracingRound::GAME_STATUS_RUN, '', $extends, '');
    ...
    private function anchorSplit()
    {
        ...
        foreach ($stake as $i){
            ...
            if ($info) {
                //$anchor_split=$i['num']*DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE;
                ...
                $anchor[]=[
                    ...
                    'amount'=>$anchor_split,
                    'liveid'=>$i['liveid'],
                    'type'=>DAOHorseracingLog::TYPE_ANCHOR,
                ];
            }else{
                //没有直播则不分
            }
    ...
    private function stakeSplit()
    {
        ...
        foreach ($stake_info as $i){
            //$stake_split=$i['amount']*(1-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE)+$i['amount'];
            ...
            $stake[]=[
                ...
                'type'=>DAOHorseracingLog::TYPE_STAKE,
            ];
        }
        return $stake;
    }
    ...
    private function bankerSplit()
    {
        ...
        $banker_dao=new DAOHorseracingBanker();
        ...
        return[
            ...
            'type'=>DAOHorseracingLog::TYPE_BANKER,
        ];
    }
    ...
    public function settle()
    {
        ...
        try{
            foreach ($d as $j){
                ...
                Interceptor::ensureNotFalse($account_model->decrease($j['from_uid'], self::SYSTEM_JOURNAL_TYPE, $orderid, $j['amount'], self::SYSTEM_CURRENCY, "run horse 扣减临时账户[{$j['from_uid']}]={$j['amount']}钻", $extends),  ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
                Interceptor::ensureNotFalse($account_model->increase($j['to_uid'], self::SYSTEM_JOURNAL_TYPE, $orderid, $j['amount'], self::SYSTEM_CURRENCY, "run horse 增加账户[{$j['to_uid']}]={$j['amount']}钻", $extends), ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
            }
    ...
    private function getGlobalTop3()
    {
        ...
        foreach ($stake as &$i){
            ...
            //$i['win_amount']=round($i['num']*(1-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE));
        ...
        $key="horseracing_top3_".$this->roundid;
        $expire=90;
    ...
    public function gameOver()
    {
        ...
        $round_dao=new DAOHorseracingRound();
        $result=$round_dao->updateRound($this->roundid, '', '', '', DAOHorseracingRound::GAME_STATUS_DIVIDED, $this->orderid, [], $endtime);
    ...
    private function settleMsgToUser()
    {
        ...
        foreach ($stake as $i){
            if (!isset($data[$i['uid']])) {
                $data[$i['uid']]=$i;
                //$data[$i['uid']]['win_amount']=$i['trackno']==$winno?$i['amount']+$i['amount']*(1-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE):0;
                $data[$i['uid']]['win_amount']=$i['trackno']==$winno?$this->hstake($i['amount']):0;
                continue;
            }
            $data[$i['uid']]['amount']+=$i['amount'];
            if ($i['trackno']==$winno) {
                //$data[$i['uid']]['win_amount']+=$i['amount']+$i['amount']*(1-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE);
        ...
        if ($data) {
            foreach ($data as $k=>$v){
                $diamond=Account::getBalance($k, Account::CURRENCY_DIAMOND);
                $options=[
                    'uid'=>$k,
                    'roundid'=>$this->roundid,
                    'amount'=>$v['amount'],
    ...
    public function settleMsgToAllLiveOutOfMax()
    {
        ...
        $largeAmount=$largeAmount/(1-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE);
            ...
            foreach ($stake as $i){
                ...
                if ($user_info) {
                    $options=[
                        ...
                        'amount'=>$i['num'],
                       // 'win_amount'=>floor($i['num']*(1-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE)),
                    ...
                    Track::showTrackHorse($i['uid'], $options['win_amount']);
                }
        ...
        if($stake) {
            ...
            foreach ($stake as $i){
                ...
                foreach ($contribute3 as &$n){
                    ...
                    //$n['win_amount']=round($n['num']*(1-DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE-DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE));
    ...
    public function recordSettle()
    {
        ...
        $orderid=$this->orderid;
        $roundid=$this->roundid;
        $loseno=$this->winno==DAOHorseracingRound::TRACK_ONE?DAOHorseracingRound::TRACK_TWO:DAOHorseracingRound::TRACK_ONE;
        //失败者
        $stake_dao=new DAOHorseracingStake();
        $stake_loser=$stake_dao->getAllStakeOfWin($roundid, $loseno);

        $log_dao=new DAOHorseracingLog();
        $log_dao->startTrans();
        try{
            //loser
            foreach ($stake_loser as $i){
                $log_dao->addLog($roundid, $i['uid'], $orderid, DAOHorseracingLog::TYPE_STAKE, $i['liveid'], $i['amount'], 0, $i['trackno']);
            }

            $split=$this->split;
            //system
            $system=$split['system'];
            $log_dao->addLog($roundid, self::SYSTEM_DIVIDE_ACCOUNT, $orderid, DAOHorseracingLog::TYPE_SYSTEM, 0, 0, $system['amount']);
            //anchor
            $anchors=$split['anchor'];
            foreach ($anchors as $i){
                $log_dao->addLog($roundid, $i['to_uid'], $orderid, DAOHorseracingLog::TYPE_ANCHOR, $i['liveid'], 0, $i['amount'], 0, "N");
            }
            //winer
            $winer=$split['winer'];
            foreach ($winer as $i){
                $log_dao->addLog($roundid, $i['to_uid'], $orderid, DAOHorseracingLog::TYPE_STAKE, $i['liveid'], $i['orginal_amount'], $i['amount'], $this->winno);
            }
            //banker
            $banker=$split['banker'];
            $log_dao->addLog($roundid, $banker['to_uid'], $orderid, DAOHorseracingLog::TYPE_BANKER, $banker['liveid'], $banker['orginal_amount'], $banker['amount']);
    ...
    private function getWinTrackNo()
    {
        ...
        $keys=range(1, 100);
        $init_track=$most_amount_track==DAOHorseracingRound::TRACK_ONE?DAOHorseracingRound::TRACK_TWO:DAOHorseracingRound::TRACK_ONE;
        $pad_track=$init_track==DAOHorseracingRound::TRACK_ONE?DAOHorseracingRound::TRACK_TWO:DAOHorseracingRound::TRACK_ONE;
    ...
    {
        $operation_dao=new DAOGameOperation();
    ...
    {
        return floor(bcmul($a, DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE, 2));
    }

    //主播分账帮助函数
    private function hanchor($a)
    {
        return floor(bcmul($a, DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE, 2));
    }
    //压住分账帮助函数
    private function hstake($a,$f=true)
    {
        $r=bcsub(1, DAOHorseracingRound::GAME_ANCHOR_DIVIDE_RATE, 2);
        $r=bcsub($r, DAOHorseracingRound::GAME_SYSTEM_DIVIDE_REATE, 2);
        $r=bcmul($r, $a, 2);
        if ($f==true) {
            $r=bcadd($r, $a, 2);
        }
    ...
    {
        return '/home/dream/codebase/service/src/application/process/game/horseracing/log/';
    }
}