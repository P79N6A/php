...
class RongCloudClient
{
    ...
    public function __construct()
    {
        ...
        //         $results = $config_model->getConfig("china", "rong_cloud_config", "server", '1000000000000');
        //         $config = json_decode($results['value'], true);
    ...
    public function queryChatroomUserExist($chatroomId, $userId)
    {
        try {
            ...
            Logger::log("msg_err", "queryChatroomUserExist msg err", json_decode(json_encode($e), true));
            throw new BizException($e->getCode(), $e->getMessage());
        }
    }
    ...
    {
        try {
            $result = $this->_client->Group()->create($userId, $groupId, $groupName);
            $result = json_decode($result, true);
            
            return $result["code"] == 200 ? $result : false;
        }  catch (Exception $e) {
            Logger::log("msg_err", "createGroup msg err", json_decode(json_encode($e), true));
            throw new BizException($e->getCode(), $e->getMessage());
        }
    }
    ...
    {
        try {
            $result = $this->_client->Group()->join($userId, $groupId, $groupName);
            $result = json_decode($result, true);
            
            return $result["code"] == 200 ? $result : false;
        }  catch (Exception $e) {
            Logger::log("msg_err", "joinGroup msg err", json_decode(json_encode($e), true));
            throw new BizException($e->getCode(), $e->getMessage());
        }
    }
    ...
    {
        try {
            $result = $this->_client->Group()->quit($userId, $groupId);
            $result = json_decode($result, true);
            
            return $result["code"] == 200 ? $result : false;
        }  catch (Exception $e) {
            Logger::log("msg_err", "quitGroup msg err", json_decode(json_encode($e), true));
            throw new BizException($e->getCode(), $e->getMessage());
        }
    }
    ...
    {
        try {
            $result = $this->_client->Group()->dismiss($userId, $groupId);
            $result = json_decode($result, true);
            
            return $result["code"] == 200 ? $result : false;
        }  catch (Exception $e) {
            Logger::log("msg_err", "dismissGroup msg err", json_decode(json_encode($e), true));
            throw new BizException($e->getCode(), $e->getMessage());
        }
    }
}
