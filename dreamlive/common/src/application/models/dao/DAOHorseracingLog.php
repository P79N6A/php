 ...
 * 记录每一局游戏的所有参与者，的资金变动
 */
class DAOHorseracingLog extends DAOProxy
{
    ...
    public function __construct()
    {
        ...
        $this->setTableName("horseracing_log");
    }
    ...
    {
        $sql    = "select a.liveid,sum(a.amount)as num,count(distinct a.uid) as people from {$this->getTableName()} a 
where a.liveid!=0 and a.type in (1,2) and liveid!=0 and a.addtime > ? and a.addtime < ? group by a.liveid";
    ...
    public function getUserSumWinAmount($start_time,$end_time)
    {
        $sql    = "SELECT SUM(result_amount-amount) AS num,uid FROM {$this->getTableName()} WHERE liveid!=0 AND result_amount>0 AND amount>0 and result_amount-amount>0 and addtime > ? and addtime < ? GROUP BY uid order by num desc limit 10";
