...
class DAOConfig extends DAOProxy
{
    ...
    public function getConfigs($region, array $names, $platform, $min_version, $max_version)
    {
        ...
        $sql = 'select * from ' . $this->getTableName() . ' where `region` = ? and `name` in (' . $placeholder . ') and `platform` = ? and min_version <= ? and (max_version >= ? or max_version = 0) order by min_version desc, id desc';
