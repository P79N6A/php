...
class GameController extends BaseController
{
    ...
    public function addAction()
    {
        ...
        $gameid = $dao_game->addGame($name, $icon, $type, $isshow, $extends, $h5_url, $weight);
        if($gameid) {
    ...
    public function updateBaseInfoAction()
    {
        ...
        $dao_game = new DAOGame();
        $row = $dao_game->updateBaseInfo($gameid, $name, $icon, $type, $isshow, '', $h5_url, $weight);
        if($row) {
            $gameInfo   = $dao_game->getGameInfos();
            $cache      = Cache::getInstance("REDIS_CONF_CACHE");
            $cache ->set('horseracing_gamelist', json_encode($gameInfo));
