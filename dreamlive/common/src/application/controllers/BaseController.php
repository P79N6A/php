...
class BaseController
{
    ...
    public function init()
    {
        ...
        $app_name = trim(strip_tags($this->getParam('lat')));
        ...
        Interceptor::ensureNotFalse(in_array($platform, array("android", "ios", "server", "obs", "ws", "cdn")), ERROR_PARAM_PLATFORM_INVALID);
        ...
        if(in_array(AUTH_CHECK_LOGIN, $auth_conf)) {
            ...
            //找回老用户 新用户token 切换为老用户
            if(version_compare($version, '2.2.4') <= 0) {
                $loginUid = Session::getLoginId($token);
                $daoMerge = new DAOUserMergeOld();
                $mergeResult = $daoMerge->getOldUserByUid($loginUid);
                if($mergeResult && $mergeResult['status'] == 'SUCCESS' && $mergeResult['olduid']>0 && !in_array($loginUid, array(10248414,10215313))) {
                    $loginUid = $mergeResult['olduid'];
                    Context::set("userid", $loginUid);
                }
            }
            //
        }
