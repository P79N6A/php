...
class AccountController extends BaseController
{
    ...
    public function activeTransferAction()
    {
        ...
        try {
            ...
            $diamond = $account->getBalance(Account::COMAPNY_ACCOUNT, ACCOUNT::CURRENCY_DIAMOND);
            Interceptor::ensureNotEmpty($diamond, ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
            Interceptor::ensureNotFalse($diamond>=$num, ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);

            $orderid = Account::getOrderId(Account::COMAPNY_ACCOUNT);
            Interceptor::ensureNotFalse($account->decrease(Account::COMAPNY_ACCOUNT, 15, $orderid, $num, ACCOUNT::CURRENCY_DIAMOND, "1000往{$active_account}活动帐户转钻$num", array()),  ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
            Interceptor::ensureNotFalse($account->increase($active_account, ACCOUNT::TRADE_TYPE_ACCOUNT_TRANS, $orderid, $num, ACCOUNT::CURRENCY_DIAMOND, "1000往{$active_account}活动帐户转钻$num", array()), ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
    ...
    public function starTransferDiamondAction()
    {
        ...
        try {
            ...
            $coin = $account->getBalance($userid, ACCOUNT::CURRENCY_COIN);
            Interceptor::ensureNotEmpty($coin, ERROR_BIZ_PAYMENT_STAR_BALANCE_DUE);
            Interceptor::ensureNotFalse($coin>=$num, ERROR_BIZ_PAYMENT_STAR_BALANCE_DUE);

            $orderid = Account::getOrderId($userid);
            $accept_num = $num/100;
            Interceptor::ensureNotFalse($account->decrease($userid, ACCOUNT::TRADE_TYPE_COIN_TO_DIAMOND, $orderid, $num, ACCOUNT::CURRENCY_COIN, "{$userid}星光买星钻100:1定比. 数量$num", array()),  ERROR_BIZ_PAYMENT_STAR_BALANCE_DUE);
            Interceptor::ensureNotFalse($account->decrease(ACCOUNT::ACTIVE_ACCOUNT2000, ACCOUNT::TRADE_TYPE_COIN_TO_DIAMOND, $orderid, $accept_num, ACCOUNT::CURRENCY_DIAMOND, "{$userid}星光买星钻100:1定比. 数量$num", array()),  ERROR_BIZ_PAYMENT_STAR_BALANCE_DUE);
            Interceptor::ensureNotFalse($account->increase($userid, ACCOUNT::TRADE_TYPE_COIN_TO_DIAMOND, $orderid, $accept_num, ACCOUNT::CURRENCY_DIAMOND, "{$userid}星光买星钻100:1定比. 数量$accept_num", array()), ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);
            $dao_star_diamond_log->addStartDiamondlog($userid, $num, $accept_num);

            Interceptor::ensureNotFalse($account->increase($userid, ACCOUNT::TRADE_TYPE_COIN_TO_DIAMOND, $orderid, $accept_num, ACCOUNT::CURRENCY_DIAMOND, "{$userid}星光买星钻100:1定比. 数量$accept_num", array()), ERROR_BIZ_PAYMENT_DIAMOND_BALANCE_DUE);

            Interceptor::ensureNotFalse($dao_star_diamond_count->insert($userid, $amount, 100), ERROR_BIZ_PAYMENT_STAR_DIAMOND_LIMIT);
