...
class liveStartPushWorker
{
    ...
    static public function execute($value)
    {
        ...
        if (!self::isSend($author["uid"], $fid, $liveid, $pushid, $reason, $type)) {
    ...
    public static function isSend($uid, $fid, $liveid, &$pushid, &$reason, $type)
    {
        if(empty($uid)) {
            return false;
        }
        ...
        //         $cache = Cache::getInstance("REDIS_CONF_CACHE");
        //         $key = "start:push:".$liveid.":".$fid.":" .$type;
...
try {
    ...
    $process->addWorker("live_start_broadcast_push",  array("liveStartPushWorker", "execute"),  10, 200);
    $process->run();
} catch (Exception $e) {
    Logger::log("process_error", "live_start_broadcast_push", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
}
