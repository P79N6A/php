    ...
    public static function execute($value)
    {
        ...
        if($type=='func:joinchatroommsgcontent') {
            ...
            try {
                ...
                $bool = $cache->ZADD($online_user_key, $liveid, $uid);
                ...
                Logger::log("online_error", "get", array("type" => $type, "uid" => $uid, "liveid" => $liveid, "bool"=> $bool));
            } catch (Exception $e) {
                Logger::log("online_error", "error", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
            }
            try {
                //写入数据表
                $option = array(
                ...
                Logger::log("process_error", "live_receive_audience_online1", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
            }
        }
...
try {
    ...
    Logger::log("process_error", "live_receive_audience_online", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
}
?>
