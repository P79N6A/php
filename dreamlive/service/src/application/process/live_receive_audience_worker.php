    ...
    if (!empty($json_array['results'])) {
        ...
        foreach ($json_array['results'] AS $key => $value) {
            ...
            case "func:joinchatroommsgcontent"://加入直播间
                ...
                //@file_put_contents('/home/dream/codebase/service/src/www/1.txt',print_r($value,true)."1\r\n",FILE_APPEND);
                ...
                //@file_put_contents('/home/dream/codebase/service/src/www/1.txt',print_r($value,true)."2\r\n",FILE_APPEND);
                //用户看播列表
                $option = array(
                            'uid'    => $value['fromUserId'],
                ...
                if (in_array($value['toUserId'], [1111])) {
                    Messenger::sendLiveJoinMessage($value['toUserId'], $value['fromUserId'], '加入了直播间', Messenger::MESSAGE_TYPE_CHATROOM_JOIN, '', []);
                    $liveinfo_info = ['uid' => 1000];
                    $info = array(
                    "liveid"     => $value['toUserId'],
                    "userinfo"     => json_encode($userinfo),
                    "liveinfo"     => json_encode($liveinfo_info),
                ...
                if ($liveinfo['status'] == Live::ACTIVING && $liveinfo['uid'] != $value['fromUserId']) {
                    ...
                    if (!empty($privacy) && isset($privacy['privacyid'])) {
                        ...
                        if ($live_num < 30 && empty($sex_zhubo)) {
                             $watchs_total = Counter::increase(Counter::COUNTER_TYPE_LIVE_WATCHES, $value['toUserId'], rand(1, 10));
                             ProcessClient::getInstance("dream")->addTask("live_distribute_robot", array('liveid'=>$value['toUserId'], 'num' => rand(2, 10)));
                        } else {
                               $watchs_total = Counter::increase(Counter::COUNTER_TYPE_LIVE_WATCHES, $value['toUserId'], rand(1, 10));
                        }
                    }
                    ...
                    if (!$iskicked) {
                        ...
                        ProcessClient::getInstance("dream")->addTask("robot_receive", array("liveid" => $value['toUserId'],"level" => $userinfo['level'],"medal" => $userinfo['medal'][0]['medal'], "vip" => $userinfo['vip'], "source" => 1, "nickname" => $userinfo['nickname']));
