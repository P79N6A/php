...
class MatchReleaseWorker
{
    ...
    static public function execute($value)
    {
        ...
        file_put_contents('/tmp/match.txt', $giftid.'-'.$receiver, FILE_APPEND);
        if($giftid == MatchReleaseWorker::GIFTID) {
            file_put_contents('/tmp/match.txt', $giftid.'-'.$receiver, FILE_APPEND);
            Match::release(0, '送礼移除小黑屋', $receiver);
            file_put_contents('/tmp/match.txt', $giftid.'-'.$receiver, FILE_APPEND);
            $content = "您被关冷宫的记录被删除，你又可以开始和小伙伴pk啦！";
...
try {
    ...
    $process->addWorker("match_release_worker",  array("MatchReleaseWorker", "execute"),  10, 2000);
    $process->run();
} catch (Exception $e) {
    ;
}
