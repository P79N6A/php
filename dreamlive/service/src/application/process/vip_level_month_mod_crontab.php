...
while(--$i>0) {
    ...
    $uid = $data['uid'];
    $level = intval(Vip::calcUserVipLevel($data['consume_current'], $data['consume_keep']));
    ...
    if($flag == DOWNLEVEL) {
        try {
            ...
            $daoMonthlyLog = new DAOUserVipMonthlyLog();
            $daoMonthlyLog->addLog($uid, json_encode($data), json_encode($info), 'down');

            $dao->commit();
        } catch (Exception $e) {
            $dao->rollback();
            $dao->modUserVipRecounttime($uid);

            $daoMonthlyLog = new DAOUserVipMonthlyLog();
            $daoMonthlyLog->addLog($uid, json_encode($data), json_encode($info), 'error_down');
        }
        ...
        if($level>0) {
            try {
                ...
                $daoVipLog = new DAOUserVipLog($uid);
                $daoVipLog->addLog($uid, $level, $level);
        
                $daoMonthlyLog = new DAOUserVipMonthlyLog();
                $daoMonthlyLog->addLog($uid, json_encode($data), json_encode($info), 'keep');
                ...
                $daoMonthlyLog = new DAOUserVipMonthlyLog();
                $daoMonthlyLog->addLog($uid, json_encode($data), json_encode($info), 'error_keep');
            }
            ...
            $dao = new DAOUserVip();
            $dao->modUserVip($uid, $info);
            Logger::log("vip_level_month", "ZEROLEVEL", array("uid"=>$data['uid'], "level"=>$level, "newlevel"=>$level, "old_consume_current"=>$data['consume_current'], "old_consume_keep"=>$data['consume_keep'], 'addtime'=>time()));
        }
    }
}
