...
class LiveRobotDistributeWorker
{
    ...
    static public function execute($value)
    {
        ...
        if (empty($liveid)) {
            Logger::log("distribute_praise", "no liveid", array("liveid"=>$liveid, "join_time"=>date("Y-m-d H:i:s", time()),"quit_time"=>date("Y-m-d H:i:s", time()), "userid"=>$liveid));
            return true;
        }
        ...
        try {
            ...
            if (empty($robot_total)) {
                Logger::log("distribute_praise", "no robots", array("liveid"=>$liveid, "join_time"=>date("Y-m-d H:i:s", time()),"quit_time"=>date("Y-m-d H:i:s", time()), "userid"=>$liveid));
                return true;
            }
            ...
            foreach($robots as $index => $userid) {
                ...
                //Logger::log("distribute_praise", "push succ", array("liveid"=>$liveid, "join_time"=>date("Y-m-d H:i:s", $join_time),"quit_time"=>date("Y-m-d H:i:s", $quit_time), "userid"=>$userid));
            }
        } catch (Exception $e) {
            Logger::log("process_error", "live_distribute_robot_error", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
            return true;
        }
...
try {
    ...
    Logger::log("process_error", "live_distribute_robot", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
}
?>
