...
class liveStartDistributeWorker
{
    ...
    static public function execute($value)
    {
        ...
        Logger::log("live_start_distribute", "get task" . $type, array_merge($value["params"], array("uid"=>$author["uid"], "line"=>__LINE__)));
        ...
        Logger::log("live_start_distribute", "get follower num succ" . $type, array("liveid"=>$liveid, "uid"=>$author["uid"], "follow_num"=>count($followers), "line"=>__LINE__));
        ...
        if (!$followers) {
            ...
            Logger::log("live_start_distribute", "follower empty" . $type, array("liveid"=>$liveid, "uid"=>$author["uid"], "follower"=>count($followers), "line"=>__LINE__, "consume"=>$consume));
        }
        ...
        foreach ($followers as $follower) {
            ...
            if (! self::isActive($fid)) {
                ...
                ProcessClient::getInstance("dream")->addTask("live_admin_start_push", $log_info);
                Logger::log(
                    "live_start_distribute", "user not active" . $type, array("liveid"=>$liveid,
            ...
            Logger::log(
                "live_start_distribute", "OK" . $type, array("liveid"=>$liveid, "uid"=>$author["uid"],"fid"=>$fid, "text"=>$text,
...
try {
    ...
    Logger::log("process_error", "live_start_broadcast_distribute", array("errorcode" => $e->getCode(), "errormsg" => json_encode($e->getMessage())));
}
