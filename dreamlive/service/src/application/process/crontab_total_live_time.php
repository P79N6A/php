...
if($live_info) {
    foreach($live_info as &$val)
    {
        ...
        $m1      = floor($val['livetime']/60);
        $rank_info[$val['etime']][$val['uid']]     += ($m1 - ($m1%30));
        unset($m1, $m, $val['livetime'], $one_time, $val);
    }
}
//过滤直播时长小于3小时的用户
if($rank_info) {
...
if($date_user) {
    ...
    foreach($date_user as $k=>$v)
    {
        ...
        if(in_array($v, $del_array)) { unset($date_user[$k]);
        }
        /* if(in_array($v,$uids))
         {
...
function sendMessage($uid, $str)
{
    //return true;//测试
    $title = '主播时长榜单通告';
    //   Messenger::sendSystemPublish(Messenger::MESSAGE_TYPE_BROADCAST_SOME, $uid, $title, $str);
}
