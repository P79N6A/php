...
class DefriendController extends BaseController
{
    ...
    const FORBIDDEN_PART_BIGLIVE_KEY = 'forbidden_part_biglive_key';
    ...
    public function bigLivePartHideAction()
    {
        $platform = trim(strip_tags($this->getParam("plat", "android")));
        Interceptor::ensureNotFalse(in_array($platform, array('ios', 'android')), ERROR_PARAM_INVALID_FORMAT, 'platform');

        $cache = Cache::getInstance("REDIS_CONF_FORBIDDEN");
        $cache->set(self::FORBIDDEN_PART_BIGLIVE_KEY."_{$platform}", 1);

        $this->render();
    }

    public function bigLivePartShowAction()
    {
        $platform = trim(strip_tags($this->getParam("plat", "android")));
        Interceptor::ensureNotFalse(in_array($platform, array('ios', 'android')), ERROR_PARAM_INVALID_FORMAT, 'platform');
        $cache = Cache::getInstance("REDIS_CONF_FORBIDDEN");
        $cache->del(self::FORBIDDEN_PART_BIGLIVE_KEY."_{$platform}");

        $this->render();
    }

    public function isBigPartLiveAction()
    {
        $platform = trim(strip_tags($this->getParam("plat", "android")));
        Interceptor::ensureNotFalse(in_array($platform, array('ios', 'android')), ERROR_PARAM_INVALID_FORMAT, 'platform');
        $cache = Cache::getInstance("REDIS_CONF_FORBIDDEN");
        $big_live_list = $cache->get(self::FORBIDDEN_PART_BIGLIVE_KEY."_{$platform}");
        $is_big_live = !empty($big_live_list) ? true : false;
