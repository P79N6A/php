...
class Match
{
    ...
    public static function apply($uid, $invitee, $duration)
    {
        ...
        //验证用户收入是否大于2000星票
        $uid_amount     = (int)Counter::get(Counter::COUNTER_TYPE_PAYMENT_RECEIVE_GIFT, $uid);
        Interceptor::ensureNotEmpty($uid_amount>=2000, ERROR_MATCH_INVITER_TICKET_FALSE);
        if($invitee>0) {
            $invitee_amount = (int)Counter::get(Counter::COUNTER_TYPE_PAYMENT_RECEIVE_GIFT, $invitee);
            Interceptor::ensureNotEmpty($invitee_amount>=2000, ERROR_MATCH_INVITEE_TICKET_FALSE);
        }
        //时长类型
        $config     = new Config();
    ...
    public static function accept($uid, $matchid)
    {
        ...
        $uid_amount     = (int)Counter::get(Counter::COUNTER_TYPE_PAYMENT_RECEIVE_GIFT, $uid);
        Interceptor::ensureNotEmpty($uid_amount>=2000, ERROR_MATCH_INVITEE_TICKET_FALSE);
        //pk双方不能是一个人
        ...
        try{
            ...
            $re1 = User::reload($match_info['inviter']);
            Logger::log("pk_match_worker_log", "accept", array("inviter"=>$match_info['inviter'],"result" => $re1));
            $re2 = User::reload($uid);
            Logger::log("pk_match_worker_log", "accept", array("uid"=>$uid,"result" => $re2));
            $live = new Live();
        ...
        $re3 = Messenger::sendToGroup($cache->zScore($living_user_key, $uid), Messenger::MESSAGE_TYPE_USER_ACCEPT_MATCH, Messenger::KEFUID, "通知受邀者直播间用户更新pk信息", $extends);
        Logger::log("pk_match_worker_log", "accept", array("medal"=>json_encode($userinfo['medal']),'userinfo'=>json_encode($userinfo),"liveid"=>$cache->zScore($living_user_key, $uid),"result" => $re3,'uid'=>$uid,'matchid'=>$match_info['matchid']));
        $re4 = Messenger::sendToGroup($cache->zScore($living_user_key, $match_info['inviter']), Messenger::MESSAGE_TYPE_USER_ACCEPT_MATCH, Messenger::KEFUID, "通知发起者直播间用户更新pk信息", $extends1);
        Logger::log("pk_match_worker_log", "accept", array("medal"=>json_encode($userinfo1['medal']),"liveid"=>$cache->zScore($living_user_key, $match_info['inviter']),"result" => $re4,'inviter'=>$match_info['inviter'],'matchid'=>$match_info['matchid']));
    }
    //pk广场
    ...
    public static function release($prisonid, $note, $uid=0)
    {
        ...
        try{
            if($prisonid) {
                ...
                file_put_contents('/tmp/match.txt', $uid.'-'.$note, FILE_APPEND);
        ...
        if($list) {
            foreach($list as $val)
            {
                ...
                $lists[]        = array(
                    ...
                    'inviter_medal'     => $inviter_userinfo['medal'],
                    'invitee'           => $val['invitee'],
                    ...
                    'invitee_medal'     => $invitee_userinfo['medal'],
                );
            }
        }
        return $lists;
    ...
    public static function sendMatch($uid, $score)
    {
        ...
        $amount     = floor($score/100)*100*0.02;
        if($amount>500) { $amount = 500;
        }
        //$num        = floor($score/100)*100*0.01;
        Logger::log("pk_match_worker_log", "sendpackeg", array("uid"=>$uid,"liveid" => $liveid,"score"=> $score));
        ActChristmas::sendRedPacketPK($liveid, $amount);
        Logger::log("pk_match_worker_log", "sendpackeg2", array("uid"=>$uid,"liveid" => $liveid,"score"=> $score));
        //给主播发礼物
        /*$giftUniTag = strval(time());
        $gift       = new Gift();
        $gift->sendGiftProcess(ActChristmas::ACT_CHRISTMAS_MAN,$uid,$giftid,$num,$liveid,1, $giftUniTag);
        Logger::log("pk_match_worker_log", "sendpackeg2", array("liveid" => $liveid,"num"=> $num,'giftid'=>$giftid,'uid'=>$uid));*/
        //更新胜利场数
        $rank   = new Rank();
