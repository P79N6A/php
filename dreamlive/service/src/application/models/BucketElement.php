...
class BucketElement
{
    ...
    public function set($bucket_name, $relateid, $type, $score, $extends = array())
    {
        ...
        try {
            ...
            $content_cache_value = "{$type}_{$relateid}";
            $cache->zAdd($content_cache_key, $score, $content_cache_value);

            if($extends) {
                $cache_extends = !is_array($extends) ? $extends : json_encode($extends);
                $cache->hset($extend_cache_key, $content_cache_value, $cache_extends);
            }
    ...
    public function import($bucket_name, $data)
    {
        ...
        try {
            ...
            foreach($data as $v){
                ...
                //     'relateid'=>$v['relateid'],
                // );
                $content_cache_value = "{$v['type']}_{$v['relateid']}";
                $cache->zAdd($content_cache_key, $v['score'], $content_cache_value);
    ...
    public function fetch($bucket_name, $offset, $num, $paging=DAOBucketElement::PADING_LIMIT)
    {
        ...
        foreach($elements as $key=>$score){
            ...
            $element_info['type'] = $tmp_key[0];
            $element_info['relateid'] = $tmp_key[1];
    ...
    public function delete($bucket_name, $relateid, $type)
    {
        ...
        try {
            ...
            $content_cache_value = "{$type}_{$relateid}";
            $cache->zRem($content_cache_key, $content_cache_value);
    ...
    public function exists($bucket_name, $type, $relateid)
    {
        ...
        $extends = !empty($extends) ? json_decode($extends, true) : '';
