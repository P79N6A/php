...
class Account
{
    ...
    const ACTIVE_ACCOUNT14 = 1014; //老用户回call.
    ...
    const ANSWER_ACTIVE_ACCOUNT = 4001; //答题渠道送现金帐号10元
    ...
    const ACTIVE_ACCOUNT9000 = 9000; //新版跑马游戏中间账号
    ...
    const TRADE_TYPE_WECHAT_ACTIVE        = 131;//微信小程序分享送钻
    ...
    const TRADE_TYPE_GAME_HANDBOOKING   = 182;//新版跑马游戏
    ...
    const TRADE_TYPE_ANSWER_ACTIVE       = 61;//答题送现金10元活动

    const TRADE_TYPE_VIP_SEND_RIDE       = 81;//商城vip送座驾对应帐号1115

    const TRADE_TYPE_GAME_RACE_HORSE    = 90;//赛马游戏
    ...
    public static function decrease($uid, $type, $orderid, $amount, $currency, $remark, $extends = array())
    {
        ...
        Interceptor::ensureNotFalse($dao_account->decrease($uid, $currency, $amount), ERROR_BIZ_PAYMENT_ACCOUNT_BALANCE_LACK, self::$ACCOUNT_TYPE_KEY[$currency]);
    ...
    public function getRevenueLog($uid, $offset, $num, $currency, $startime, $endtime, $type)
    {
        ...
        foreach ($journal_list as $key => $value) {
            ...
            if($currency == 1) {//星钻
                $dao_journal    = new DAOJournal($extend['sender']);
            }elseif($currency == 5) {//星光
                $dao_journal    = new DAOStarJournal($extend['sender']);
            }
            $journal_list[$key]['diamond']    = $dao_journal -> getStarOne($extend['sender'], $value['orderid'])?$dao_journal -> getStarOne($extend['sender'], $value['orderid']):0;
            unset($extend);
        }
        $more = 'Y';
