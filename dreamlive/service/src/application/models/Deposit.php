...
class Deposit
{
    ...
    const DEPOSIT_WECHAT_GOPAY = "wechatGopay";
    const DEPOSIT_BANK_GOPAY = "bankGopay";
    const DEPOSIT_TMALL = "tmall";

    public static function getClient($source)
    {
        Interceptor::ensureNotEmpty(in_array($source, array(self::DEPOSIT_WECHAT, self::DEPOSIT_WECHAT_H5, self::DEPOSIT_ALIPAY, self::DESOSIT_GOOGLE, self::DEPOSIT_PAYPAL, self::DEPOSIT_APPLE, self::DEPOSIT_MYCARD, self::DEPOSIT_BANK_GOPAY, self::DEPOSIT_WECHAT_GOPAY, self::DEPOSIT_TMALL)), ERROR_BIZ_PAYMENT_DEPOSIT_SOURCE_ERROR, $source);
    ...
    public static function prepare($uid, $source, $currency, $amount, $openid)
    {
        return self::getClient($source)->prepare($uid, $source, $currency, $amount, $openid);
    }
    ...
    public static function complete($orderid, $tradeid, $source="")
    {
        ...
        if($source == self::DEPOSIT_WECHAT || $source == self::DEPOSIT_ALIPAY || $source == self::DEPOSIT_APPLE || $source == self::DEPOSIT_BANK_GOPAY || $source == self::DEPOSIT_TMALL) {
        ...
        try {
            ...
            //$rt=$dao_deposit->getDepositInfo($orderid);
            //if ($rt['status']!="Y")throw new Exception("订单状态异常！");
            ...
            Interceptor::ensureNotFalse($dao_deposit->setDepositStatus($deposit_info['orderid'], 'Y', $tradeid), ERROR_BIZ_PAYMENT_DEPOSIT_FALSE, $deposit_info['orderid']);
        ...
        if($source == self::DEPOSIT_TMALL) {
            $config = new Config();
            $config_info = $config->getConfig('china', "payment_deposit_tmall_task", "server", '1.0.0.0');
            $arr = explode(";", $config_info['value']);
            foreach ($arr as $key => $value) {
                $tmp = explode("|", $value);
                if((int)$deposit_info['amount'] == $tmp[0]) {
                    ProcessClient::getInstance("dream")->addTask("passport_task_execute", array("uid" => $deposit_info['uid'], "taskid" => $tmp[1], "num" => 1, "ext"=>json_encode(array('amount'=>$tmp[0]*10))));
                }
            }
