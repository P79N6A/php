...
class Product
{
    ...
    {
        $cache = Cache::getInstance("REDIS_CONF_PAYMENT");//REDIS_CONF_PAYMENT
    ...
    {
        if (empty($cateids)) {
            $cateids=[DAOProduct::CATEID_RIDE,DAOProduct::CATEID_BIG_HORN,DAOProduct::CATEID_SMALL_HORN,DAOProduct::CATEID_ANCHOR_LEVEL_TOKEN];
        }
    ...
    private static function storeListProcess($products)
    {
        ...
        if (is_array($re[DAOProduct::CATEID_RIDE])) { $result[]=array('name'=>'进场特效','list'=>$re[DAOProduct::CATEID_RIDE]);
        }
        if (is_array($re[DAOProduct::CATEID_ANCHOR_LEVEL_TOKEN])) { $result[]=array('name'=>'荣誉勋章','list'=>$re[DAOProduct::CATEID_ANCHOR_LEVEL_TOKEN]);
        }
        ...
        if (is_array($re[DAOProduct::CATEID_BIG_HORN])||is_array($re[DAOProduct::CATEID_SMALL_HORN])) { $result[]=array('name'=>'系统道具','list'=>array_merge($bigHorns, $smallHorns));
        }
