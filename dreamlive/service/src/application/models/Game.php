...
class Game
{
    ...
    public static function getGameList()
    {
        ...
        if($gameInfo) {
            foreach($gameInfo as &$value){
                ...
                $value['h5_url']        =self::com($value['h5_url']);
            }
        }

        return $gameInfo;
    }
    private static function com($u)
    {
        $p=Context::get('platform');
        $v=Context::get('version');
        if ($p=='ios'&&version_compare($v, '3.2.7.0', '<=')) {
            return "http://html5.tongyigg.com/game/lotto/index.html";
        }
        return $u;
    }
    /**
    ...
    public static function getHandbookingerGame()
    {
        return self::getGameByType(DAOGame::TYPE_HANDBOOKINGER);
    }

    public static function updateRoomid($gameid,$roomid)
    {
        $daoGame=new DAOGame();
        return $daoGame->updateRoomid($gameid, $roomid);
    }
    ...
    public static function generateGame($gameid,$type)
    {
        $dao_game = new DAOGame();
        return $dao_game->generateGame($gameid, $type);
    }
    
    public static function addGame($name,$icon,$type,$isshow,$extends,$h5_url,$weight,$fullscreen)
    {
        $dao_game = new DAOGame();
        $dao_game->startTrans();
        try{
            $gameid = $dao_game->addGame($name, $icon, $type, $isshow, $extends, $h5_url, $weight, $fullscreen);
            $dao_game->generateGame($gameid, $type);
            $dao_game->commit();
            return $dao_game->getGameInfos($gameid);
        }catch (Exception $e){
            $dao_game->rollback();
            throw $e;
        }
