...
class DAOLinkApply extends DAOProxy
{
     ...
     * @param int $liveid
     */
    public function closeLinkApply($uid, $relateid, $liveid)
    {
        $condition = ' uid=? and relateid=? and liveid=?  and status='.Link::LINK_APPLY_CONNECT;
        $params = array(
            'uid'      => $uid,
            'relateid' => $relateid,
            'liveid'   => $liveid
