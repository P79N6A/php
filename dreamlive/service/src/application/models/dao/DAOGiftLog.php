...
class DAOGiftLog extends DAOProxy
{
    ...
    //红蓝大战
    public function getGiftActTopN($giftid,$start,$end,$direct='R',$uids=array(),$lines=3)
    {
        $ufield=$direct=="S"?"sender":"receiver";
        $where="";
        if (!empty($uids)) {
            $blst=implode(",", $uids);
            $where=" and receiver in (".$blst.") ";
        }
        $re= $this->getAll("select sum(num*price) as total,".$ufield." as uid from ".$this->getTableName()." where giftid=? ".$where." and addtime between '".$start."' and '".$end."' group by ".$ufield." order by total desc limit ".$lines, ['giftid'=>$giftid]);
        array_walk(
            $re, function (&$v,$k) {
                $v['rank']=$k+1;
            } 
    ...
    //红蓝大战当前成绩
    public function getScore($giftid,$start,$end,$uids=array())
    {
        $where="";
        if (!empty($uids)) {
            $blst=implode(",", $uids);
        ...
        return $this->getRow("select sum(num*price) as total from ".$this->getTableName()." where giftid=? ".$where." and addtime between '".$start."' and '".$end."'", ['giftid'=>$giftid]);
    }
}
