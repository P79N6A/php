...
/*
// act_christmas
0,30 12-19,1-4 23,26-31,1 * * php /home/dream/codebase/service/src/application/process/crontab_activity_christmas.php 3
0,10,20,30,40,50 20-23,0 23,26-31,1 * * php /home/dream/codebase/service/src/application/process/crontab_activity_christmas.php 3

0,30 1-4 24,25 * * php /home/dream/codebase/service/src/application/process/crontab_activity_christmas.php 3
0,10,20,30,40,50 12-23,0 24,25 * * php /home/dream/codebase/service/src/application/process/crontab_activity_christmas.php 3
...
class ActChristmas
{
    ...
    const ACT_BUSY_TIME_END="02:00:00";//第二天 4
    const ACT_BUSY_STAT_FREQUENCY=10;//分钟 //5

    const ACT_STOP_TIME_START="02:00:00"; //4
    const ACT_STOP_TIME_END="12:00:00";//12:00:00

    const ACT_CHRISTMAS_MAN=11111111;//?  12181933

    const ACT_WISH_BAG_GIFT_ID=3768;//?
    const ACT_DEFAULT_AVG_RED_PACKET_AMOUNT=3;
    const ACT_FREE_AVG_RED_PACKET_AMOUNT=5;
    const ACT_BUSY_AVG_RED_PACKET_AMOUNT=10;
            ...
            if ($actState==self::ACT_STATE_IN_RUNNING) {
                ...
                if ($anchorid) {//主播端
                    ...
                    if ($liveid) {
                        ...
                        //给xs发警告
                        self::sendXSWarn($anchorid, $liveid);
                    }
                }
    ...
    public static function getTimeSpan()
    {
        ...
        $config=self::getTimeConfig();
        list($stopStart,$stopEnd)=$config['stop'][0];
    ...
    public static function sendRedPacketPK($liveid, $amount)
    {
        try{
            $roomPeopleNum=self::getRoomPeopleNum($liveid);
            //if (!$roomPeopleNum) throw new Exception("room people num is zero!");
            if($roomPeopleNum <= 5) { $roomPeopleNum = 5;
            }
            ...
            Logger::log("pk_match_worker_log", "sendSharePacket", array("amount"=>$amount,"num" => $num,"remark"=> $remark,"liveid"=>$liveid,"threshold"=>$threshold));
            $packetid=$packet->sendSharePacket($amount, $num, $remark, $liveid, $threshold);
            Logger::log("pk_match_worker_log", "sendSharePacket2", array("amount"=>$amount,"num" => $num,"remark"=> $remark,"liveid"=>$liveid,"threshold"=>$threshold));
            if (!$packetid) { throw new Exception("send red packet failed! ");
            }
        }catch (Exception $e){
            self::log(__FUNCTION__, get_defined_vars(), $e);
    ...
    public static function inWhichTime()
    {
        ...
        foreach ($config as $k=>$v){
            foreach ($v as $i){
                list($stime,$etime)=$i;
                $tstime=strtotime($today." ".$stime);
                $tetime=strtotime($today." ".$etime);
                if ($now>=$tstime&&$now<=$tetime) {
                    return $k;
                }
            }
        }
    ...
    public static function sendXSWarn($anchorid,$liveid)
    {
        try{
            $cache = Cache::getInstance("REDIS_CONF_CACHE");
            $xs = $cache->zRevRange("zset_push_white_list", 0, -1);
            $xs=is_array($xs)?$xs:array();
            if (in_array($anchorid, $xs)&&$liveid>0) {
                $msg="恭喜你获得了圣诞活动即时榜前三，请注意尺度，健康直播！";
                Messenger::sendLiveWarning($anchorid, $msg, $liveid);
            }
        }catch (Exception $e){
            self::log(__FUNCTION__, get_defined_vars(), json_encode($e));
        }
    }
    ...
    public static function getTimeConfig()
    {
        ...
        $day24="2017-12-24";
        $day25="2017-12-25";
        if ($today==$day24||$today==$day25) {
            return array(
                self::TIME_SPAN_FREE=>array(
                    array('01:00:00','04:00:20'),//03:59:59
                    //array('12:00:00','19:59:59'),
                ),
                self::TIME_SPAN_BUSY=>array(
                    array('00:00:00','00:59:59'),
                    array('12:00:00','23:59:59'),
                ),
                self::TIME_SPAN_STOP=>array(
                    array('04:00:21','11:59:59')//04:00:00
                ),
            );
        }
        return array(
