...
class WeekStarTopn
{
    ...
    private static function statStarWeekByWeek($week)
    {
        ...
        if ($weekStar) {
            ...
            $giftStarLogDao=new DAOGiftStarLog();
            $gift=new Gift();
            $gifts=DAOWeekStar::parseGift($weekStar['gifts']);
            foreach ($gifts as $i){
                $info=$gift->getGiftInfo($i);
                if ($info) {
                    if ($info['consume']==DAOGift::GIFT_CONSUME_DIAMOND) {
                    ...
                    }elseif ($info['consume']=DAOGift::GIFT_CONSUME_COIN) {
                        $result[$i]['receive']=$giftStarLogDao->getReceiveGiftNumTopN($i, $weekStar['week_start'], $weekStar['week_end']);
                        $result[$i]['send']=$giftStarLogDao->getSendGiftNumTopN($i, $weekStar['week_start'], $weekStar['week_end']);
                    }
    ...
    public static function getHeader($info,$uid)
    {
        ...
        if ($uid) {
            ...
            $giftDao=new DAOGift();
            $giftLogDao=new DAOGiftLog();
            $giftStarDao=new DAOGiftStarLog();
            $r=[];
            foreach ($gifts as $i){
                $giftInfo=$giftDao->getInfo($i);
                if ($giftInfo) {
                    if ($giftInfo['consume']==DAOGift::GIFT_CONSUME_DIAMOND) {
                    ...
                    }elseif ($giftInfo['consume']==DAOGift::GIFT_CONSUME_COIN) {
                        $t=$giftStarDao->getGiftRankByUid($uid, $i, $info['week_start'], $info['week_end']);
                        $r[$i]=array(
                            'giftid'=>$i,
                            'name'=>$giftInfo['name'],
                            'image'=>Util::joinStaticDomain($giftInfo['image']),
