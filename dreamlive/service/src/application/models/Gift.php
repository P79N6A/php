...
class Gift
{
    ...
    public function send($sender, $receiver, $liveid, $giftid, $consume, $price, $ticket, $num, $gift_name, $isBagGift = false)
    {
        ...
        try {
            ...
            //2018-04-08 由焦总，改为1300，原7000
            $sysAccount7000 = self::use7001($giftid) ? Account::ACTIVE_ACCOUNT7001 : Account::ACTIVE_ACCOUNT1300;
    ...
    public static function giftListProcess()
    {
        ...
        $gift_list=self::getMiniAppGiftList($gift_list);
        return $gift_list;
    }

    private static function getMiniAppGiftList($data)
    {
        $r=$data;
        ...
        if ($appName=='miniapp') {
            $d=array();
            $gifts=array(
                3793,3842,3813,3845,3819,3836,3835,3823,
            );
            foreach ($data['gift_list'] as $i){
                if (in_array($i['giftid'], $gifts)) {
                    $i['tag']=DAOGift::GIFT_TAG_INTERACTION;
                    $d[]=$i;
                }
            }
            usort(
            ...
            $r['gift_list']=$d;
            $tag_list=[
                ['name'=>"活动",'sort'=>4,'id'=>DAOGift::GIFT_TAG_INTERACTION] ,//faceu
            ];
            $r['tag_list']=$tag_list;
        }
        return $r;
    ...
    public function sendGiftProcess($sender, $receiver, $giftid, $num, $liveid, $doublehit, $giftUniTag, $isBagGift = false)
    {
        ...
        try {
            ...
            if ($gift_info['consume'] == 'DIAMOND') {//跑道消息
                Track::showTrackGift($uid, $receiver, $giftid, $num, $liveid);
                //活动
                self::actIncreaseIncome($sender, $receiver, $num, $giftid, $gift_info['price'], $gift_info['ticket']);
            }
    ...
    //活动拉收
    private static function actIncreaseIncome($sender, $receiver, $num, $giftid, $price, $ticket)
    {
        //2018-03-15（23261324,23261325,23261326,23261327,23261328,23261329,23261330,23261331,23261332,23261334,23261335,23261336,23261337,23261338,23261339,23261340,23261341,23261342,23261343,23261344）
        //2018-03-19（23260197,23259847,23259574,23260786,23261311,23260190,23260047,23259717,23259950,23260253,）
        try {
            //运营回收
            $uids = array(23261324,23261325,23261326,23261327,23261328,23261329,23261330,23261331,23261332,23261334,23261335,23261336,23261337,23261338,23261339,23261340,23261341,23261342,23261343,23261344);
            $uids =array_merge($uids, array(23260197,23259847,23259574,23260786,23261311,23260190,23260047,23259717,23259950,23260253,));
            if (!empty($uids)) {
                if (in_array($sender, $uids)) {
                    $accountInterface = new AccountInterface();
                    $accountInterface->keng($receiver, $num * $ticket, "孟旭运营拉收活动(坑土豪):sender[{$sender}],receiver[{$receiver}],giftid[{$giftid}],num[{$num}],price[{$price}],ticket[{$ticket}]");
                }
            }
        } catch (Exception $e) {
