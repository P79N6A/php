...
class ActSummer
{
    ...
    public static function sendRide($uid)
    {
        try{
            ...
            $daoActChristmasRankTotal=new DAOActChristmasRankTotal();
            $ride=$daoActChristmasRankTotal->getRichManRide($uid, $cfg['ride_list'], 4);
            if (!$ride) { return;
            }

            if (Bag::hasTheRide($uid, $ride)) { return;
            }
            $product=Product::getOne($ride);
            if (!$product) { return;
            }
            $expire=10*24*60*60;
            $notice="恭喜您获得夏日狂欢即时土豪榜第一，送给你一个进场特效，快去体验吧 ";
    ...
    public static function sendWishBag($anchorid,$liveid)
    {
        try{
            ...
            $content="恭喜您获得夏日狂欢主播即时榜第一名，您将收到一个幸运福袋和一个直播间惊喜红包！";
    ...
    public static function sendTrackMsg($uid,$anchorid)
    {
        try{
            ...
            $msg="恭喜主播（{$anchor['nickname']}）获得幸运福袋，还有惊喜红包降临直播间大家快来抢！";
            $img="";
    ...
    public static function springActConfig()
    {
        ...
        return array(
            4=>array(
                'stage'=>4,
                'name'=>'夏日狂欢',
                'stime'=>'2018-07-11 12:00:00',
                'etime'=>'2018-07-22 23:59:59',
                'gifts'=>array(3874,3800,3836,3847),
                //'gifts'=>array(3793,),
                'bagGift'=>3768,
                'bagGiftName'=>'恭喜发财',
                'busy_time'=>array('00:00:00-02:00:20','20:00:00-23:59:59'),
                'free_time'=>array('12:00:00-19:59:59'),
                //'free_time'=>array('10:00:00-19:59:59'),
                'stop_time'=>array('02:00:21-11:59:59'),
                //'stop_time'=>array('02:00:21-09:59:59'),
                'total_rank_num'=>10,
                'real_time_rank_num'=>5,
                'busy_freq'=>20,
                'free_freq'=>30,
                'total_busy_score'=>1,
                'total_free_score'=>1,
                ...
                'red_bag_amount'=>300,//最多1000钻
                'ambassador'=>23081180,
                'total_rank_key'=>'summer_act_total_rank_4',
                'real_time_rank_key'=>'summer_act_real_time_rank_4',
                'ride_list'=>array(
                    1=>array('id'=>96,'num'=>1),
                    2=>array('id'=>97,'num'=>20),
                    3=>array('id'=>98,'num'=>50),
                ),
            ),
        );
    }
    ...
    public static function w($msg)
    {
        ...
        $dstr.=json_encode($d);
        $dstr.="\n\n";
        file_put_contents($f, $dstr, FILE_APPEND);
