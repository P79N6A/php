...
class WechatPay
{
    public static function prepare($uid, $source, $currency, $amount, $openid = '')
    {
        ...
        if ($_REQUEST['type'] == 'js') {
            $openid  = $_REQUEST['openid']?$_REQUEST['openid']:$openid;
            $wx = new WxPayApi();
            ...
            $input->SetNotify_url($API_DOMAIN . "/deposit/notify_wechat_js"); //js
            ...
            $input->SetTime_start(date("YmdHis"));
            $input->SetTime_expire(date("YmdHis", time() + 600));
            $input->SetNotify_url($API_DOMAIN . "/deposit/notify_wechat");
