...
class Messenger
{
    ...
    const MESSAGE_TYPE_CHATROOM_MULTI_TYPE  = 330; //支持消息含图片的公聊消息
    ...
    public static  function sendLiveJoinMessage($liveid, $sender, $content, $type, $guard=0, $privacy = array())
    {
        ...
        "liveid" => $liveid,
        "userid"=>$sender,
        "nickname"=>empty($userinfo['nickname']) ? "1" : $userinfo['nickname'],
        "avatar"=>empty($userinfo['avatar']) ? "1": $userinfo['avatar'],
        "level"=>empty($userinfo['level']) ? "1" : $userinfo['level'],
                "anchor_token"=>$userinfo['anchor_token'],
        "medal"=>empty($userinfo['medal']) ? [] : $userinfo['medal'],
        "gender"=>$userinfo['gender'],
        "founder"=>empty($userinfo['founder']) ? "1" : $userinfo['founder'],
        ...
        if (empty($extra['nickname']) || empty($extra['avatar'])) {
            @file_put_contents('/home/dream/codebase/service/src/www/1.txt', print_r($extra, true)."{$type}\r\n", FILE_APPEND);
        }
