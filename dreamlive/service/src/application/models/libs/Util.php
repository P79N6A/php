...
class Util
{
    public static function isValidClient($userid, $deviceid, $platform, $network, $version, $rand, $netspeed, $time, $guid)
    {
        ...
        $t=$api_access_config["client"]["token"];
        $app_name=Context::get('app_name');
        if ($app_name=='miniapp') {
            $t=$api_access_config["miniapp"]["token"];
            //file_put_contents("/tmp/sign.txt",json_encode(array('kkk',$str,md5($str .$t ),$guid))."\n\n",FILE_APPEND);
        }
        
        return $guid == md5($str .$t)? true : false;
    }
    ...
    public static function isValidServer($rand, $time, $guid)
    {
        ...
        $api_access_config = Context::getConfig("API_ACCESS_CONFIG");
        if($_REQUEST['_dbg']==1) {
            Util::log("KKK:::", ['guid'=>$guid,'md5'=>md5(implode("_", $params) . $api_access_config["server"]["token"]),'ip'=>Util::getIP(),'server'=>$_SERVER], 'r4');
        }
    ...
    public static function mkServerCp($rand=1, $time=1)
    {
        $params = array(
            "rand"=>$rand,
            "time"=>$time
        ...
        $api_access_config = Context::getConfig("API_ACCESS_CONFIG");
        $guid = md5(implode("_", $params) . $api_access_config["server"]["token"]);
        return [
            'guid'=>$guid,
            'rand'=>$rand,
            'time'=>$time,
    ...
    public static function joinStaticDomain($field)
    {
        ...
        $discard2='dreamlive.cn';
        $discard3='dreamlive.com';*/
        if (empty($field)) { return '';
        }

        if (stripos($field, 't.jinmayx.com')!==false) { return $field;
        }
        ...
        $replace=false;
        $discard='';
        foreach ($discards as $i){
            if (stripos($field, 'http://static.'.$i) === 0||stripos($field, 'https://static.'.$i) === 0) {
                $replace=true;
                $discard=$i;
                break;
            }
    ...
    public static function log($key,$data,$f='fff')
    {
        try{
            $file="/tmp/".$f.".txt";
            $d=array(
