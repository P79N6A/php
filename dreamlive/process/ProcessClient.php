<?php
define("PROCESS_CLIENT_ROOT", dirname(__FILE__));
require_once PROCESS_CLIENT_ROOT . "/process_conf.php";
require_once PROCESS_CLIENT_ROOT . "/base/Job.php";

$GLOBALS["argv"] = isset($argv) ? $argv : array();
$GLOBALS["QUEUE_CONF"] = $QUEUE_CONF;
...
class ProcessClient
{
    ...
    public static function getInstance($product)
    {
        ...
        if ($process == null || ! $process instanceof ProcessClient) {
    ...
    {
        /* {{{ */
        return Job::getInstance($this->_product)->isQueueAlive();
    }
    /* }}} */
    ...
    {
        /* {{{ */
        return Job::addLog((array) $userlog);
    }
    /* }}} */
    ...
    public function getTask($job)
    {
        return Job::getInstance($this->_product, $job)->getTask();
    }
    
    public function addWorker($job, $worker, $max_children = 1, $max_task = 1000)
    {
        /* {{{ */
        $this->_worker[strtolower($job)] = array(
            "worker" => $worker,
    ...
    public function run()
    {
        ...
        $op_job = strtolower($this->_argvs[2]);
    ...
    private function _startJob($op_job = null, $is_restart = false)
    {
        ...
        $is_child_process = preg_match("/^_(\d+)_$/", $this->_argvs[4]);
